...
class FieldsetMixin(object):
    """Simple implementation of form fieldsets."""

    def __init__(self, *args, **kwargs):
        super(FieldsetMixin, self).__init__(*args, **kwargs)
        self.fieldsets = tuple(
            _initialize_fieldset(self, fs) for fs in self.fieldsets)


def _initialize_fieldset(form, fieldset):
    """
    >>> form = {'a': 'field_a', 'b': 'field_b'}
    >>> fs = ('Some fields', ('a', 'b'))
    >>> _initialize_fieldset(form, fs)
    ('Some fields', ('field_a', 'field_b'))
    """
    title, names = fieldset
    fields = tuple(form[n] for n in names)
    return title, fields
