...
from sxconsole.sx_api import Version
...
def mock_api(api):
    ...
    def modifyVolume(volname, size=None, owner=None, newName=None,
                     maxRevisions=None):
        try:
        ...
        if newName and volname != newName:
            for collection in (ACL, VOLUMES):
                collection[newName] = collection.pop(volname)
    api.modifyVolume = modifyVolume

    def changeVolumeReplica(volname, nextReplica):
        try:
            volume = VOLUMES[volname]
        except KeyError:
            raise SXClusterNotFound("Volume does not exist")
        volume['replicaCount'] = nextReplica
    api.changeVolumeReplica = changeVolumeReplica
    ...
    api.get_sx_version = lambda: Version(2, 1, True)
    api.listNodes = lambda: deepcopy(NODES)
    api.get_cluster_uuid = lambda: CLUSTER_UUID

    dummyID = ('0' * 20).encode('hex')
    api.getUser = lambda u: {'userID': dummyID, 'userKey': dummyID}
